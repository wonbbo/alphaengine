...
# ADR-011: 설정 관리 우선순위 정책

## 컨텍스트

AlphaEngine v2는 두 가지 설정 저장소를 사용한다:

1. **`secrets.yaml`**: 파일 기반, 정적, 보안 정보 저장
2. **`config_store`**: DB 기반, 동적, 런타임 설정 관리

초기에는 역할 구분 없이 두 곳에 설정이 혼재되어 우선순위가 모호했다.

특히 `strategy` 설정의 경우:
- `secrets.yaml`에서 로드 → 봇 재시작 필요
- Web UI에서 `config_store` 변경해도 반영 안 됨
- 사용자 혼란 발생
...
| 설정 분류 | 저장소 | 예시 |
|----------|--------|------|
| 보안 정보 | `secrets.yaml` | API Key, Secret, Webhook URL |
| 모드 선택 | `secrets.yaml` | `mode: testnet/production` |
| 런타임 설정 | `config_store` | 전략, 리스크, 이체, BNB 수수료 |

### secrets.yaml 담당 (정적, 보안)
...
### config_store 담당 (동적, 런타임)

| 키 | 설명 |
|---|------|
| `engine` | 엔진 모드, 폴링 주기 |
| `risk` | 리스크 관리 설정 |
| `strategy` | 전략 모듈, 클래스, 파라미터, 자동시작 |
| `strategy_state` | 전략 상태 (읽기 전용) |
| `transfer` | 이체 한도/수수료 설정 |
| `bnb_fee` | BNB 자동 충전 설정 |

## 이유

1. **역할 분리**
   - `secrets.yaml`: 보안 정보만 (git ignore)
   - `config_store`: 운영 설정 (Web UI에서 변경 가능)

2. **운영 편의성**
   - 전략 변경 시 봇 재시작 불필요
   - Web UI에서 실시간 설정 변경
   - 변경 이력 추적 (version, updated_at, updated_by)

3. **보안**
   - API Key/Secret은 DB에 저장하지 않음
   - `secrets.yaml`은 .gitignore에 포함

## 결과

- `strategy` 설정을 `secrets.yaml`에서 `config_store`로 이동
- `bootstrap.py`에서 `config_store` 우선 로드
- `secrets.yaml.example`에 역할 분리 안내 추가
- Web UI 설정 페이지에서 전략 편집 가능
